...
<?php echo("<form  method=\"POST\" style=\"margin: 50px 0;\">
...
if(isset($_GET['action']) and $_GET['action'] == 'edit'){
    ...
    $id = $_GET['id'];
    $sql = $_GET['path'] =="employees"? 'SELECT fullName FROM employees WHERE idEmployee ='.$id .'' : 'SELECT projectName FROM projects WHERE idProject = '.$id .'';
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)) {
 
    $value=   $_GET["path"] == "employees"? $row["fullName"] : $row["projectName"];
            ...
            <label class=\"mdc-text-field mdc-text-field--filled\">
                <span class=\"mdc-text-field__ripple\"></span>
                <input class=\"mdc-text-field__input\" type=\"text\" aria-labelledby=\"my-label-id\" name=\"update\" value=\"".$value."\">
                <span class=\"mdc-line-ripple\"></span>
                    </label>");
        if($_GET["path"] == "employees"){print (" <label class=\"mdc-text-field mdc-text-field--filled\">
    ...
    if (!$conn) 
        ...
        if(count($_POST)>0) {
               if(!empty($_POST['update'])){
                $sql=$_GET["path"]=="employees"? "UPDATE employees set fullName='" . $_POST['update'] . "' WHERE idEmployee= ".$_GET['id'] ."": "UPDATE projects set projectName='" . $_POST['update'] . "' WHERE idProject= ".$_GET['id'] ."";
    }
    if(!empty($_POST['add'])){
        echo ("here");
        $sql=$_GET["path"]=="employees"? "INSERT INTO employees (fullName) VALUES "."(\"". $_POST['add'] ."\");": "INSERT INTO projects (projectName) VALUES "."(\"". $_POST['add'] ."\");";
    }
